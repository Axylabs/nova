...
export function emitRegistry(m: Model, fingerprint: number, ctx: EmitContext = {}): string {
  ...
  const poolImport = userMode ? (ctx.libraryImport ?? "ignex-nova") : "../transport/byte-buffer-pool";
...
function emitLocalTypes(
  ...
  for (const ev of controlEvents) lines.push(`  ${JSON.stringify(ev.name)}: ${ev.tableName}Payload;`);
  lines.push("};");
  lines.push("");
  return lines;
}
