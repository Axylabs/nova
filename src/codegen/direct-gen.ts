...
export function emitDirectSer(m: Model, ctx: EmitContext = {}): string {
  ...
  if (userMode) {
    lines.push(`import { encodeUtf8Into, ensureCapacity, utf8Len, checkInt64 } from ${JSON.stringify(libImport)};`);
  } else {
...
function emitLocalTypes(m: Model, tables: Model["tables"], directEvents: Model["events"]): string[] {
  const lines: string[] = [];
  for (const t of tables) {
    lines.push(`export interface ${t.name}Payload {`);
    for (const f of t.fields) lines.push(`  ${f.jsonName}${f.required ? "" : "?"}: ${plainTsType(m, f)};`);
    lines.push("}");
    lines.push("");
  }
  const appNames = directEvents.filter((e) => !e.control).map((e) => `"${e.name}"`).join(" | ");
  const ctlNames = directEvents.filter((e) => e.control).map((e) => `"${e.name}"`).join(" | ");
  ...
  for (const ev of directEvents.filter((e) => !e.control)) lines.push(`  ${JSON.stringify(ev.name)}: ${ev.tableName}Payload;`);
  lines.push("};");
