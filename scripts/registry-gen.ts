 ...
 */
export function emitRegistry(m: Model): string {
  ...
  const lines: string[] = [];
  lines.push("// @generated by scripts/registry-gen.ts — DO NOT EDIT");
  lines.push('import * as flatbuffers from "flatbuffers";');
  lines.push('import { pooledByteBuffer } from "../transport/byte-buffer-pool";');
  lines.push("");
  lines.push("import {");
  ...
  lines.push('import type { AnyEventName, ControlEventName, ControlEvents, Events, EventName } from "../schema";');
  lines.push("");
  ...
  for (const ev of appEvents) lines.push(`  ${ev.name}: ${eventId(ev.name)}, // fnv1a32("${ev.name}")`);
  lines.push("};");
  lines.push("");
  lines.push("export const controlEventNameToId: Record<ControlEventName, number> = {");
  for (const ev of controlEvents) lines.push(`  ${ev.name}: ${eventId(ev.name)}, // fnv1a32("${ev.name}")`);
  lines.push("};");
  lines.push("");
  ...
  for (const ev of m.events) {
    ...
    lines.push(`  ${eventId(ev.name)}: (bb) => ${plain}ToPlain(${ev.tableName}.getSizePrefixedRootAs${ev.tableName}(bb).unpack()),`);
  }
  lines.push("};");
  lines.push("");
  ...
  lines.push("");
  lines.push("export function readFrameHeader(bytes: Uint8Array): { name: AnyEventName; id: number } | null {");
  ...
  lines.push("  const id = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength).getUint32(1, true);");
  ...
  lines.push("}");
  lines.push("");
  lines.push("export function decodeFrame(bytes: Uint8Array): { name: AnyEventName; id: number; payload: unknown } | null {");
  ...
  for (const t of neededTables) {
    ...
    const annot = event ? `: ${event.control ? "ControlEvents" : "Events"}[${JSON.stringify(event.name)}]` : "";
